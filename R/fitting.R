...
#this didn't work...so I just cut and paste the code for functions
#ecofitting_cut_paste.R
...
rsim.plot.biomass(scene0, fit.initial, test_sp[21])
...
fit.final  <- rsim.fit.run(out_values, fit_species, fit_vartype, scene0, verbose=T,
                           run_method='RK4', years=fit.years) 
rsim.plot.biomass(scene0, fit.final, test_sp[21])
...
rsim.plot.biomass(scene1, run1, test_sp[21])
rsim.plot.biomass(scene1, run1, test_sp[22])
