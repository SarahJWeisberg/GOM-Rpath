...
as.data.table(spp)
spp2 <- spp[!duplicated(spp$SVSPP),]

#Merge survdat with Rpath species & group names
GOM.prep2<-merge(GOM.prep, spp2[,list(SVSPP,COMNAME,RPATH,SCINAME)], by = 'SVSPP')
...
GOM.strat.mean <- stratmean(GOM.prep2, group.col = 'RPATH', strat.col = 'STRATUM')
...
GOM.fall.80s <- GOM.fall.80s[,sum(strat.biomass), by = RPATH]
