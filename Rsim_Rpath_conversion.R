...
#Eventually will need to loop this over all successes
REco.sense1<-REco.sense[[1]]

#Assign biomass
alt$model[,Biomass:=REco.sense1$B_BaseRef[-1]]

#Assign PB
alt$model[,PB:=REco.sense1$PBopt[-1]]

#Assign QB
alt.QB<-REco.sense1$FtimeQBOpt[-1]
...
PreyFrom<-REco.sense1$PreyFrom[-c(1,2)]
PreyTo<-REco.sense1$PreyTo[-c(1,2)]
predpreyQ<-REco.sense1$QQ[-c(1,2)]
...
for (i in 1:nliving){
  diet[,i]<-test.QQ[,i]/(alt.QB[i]*alt.biomass[i])
}
