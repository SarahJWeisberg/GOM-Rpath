...
#########Bundy Scotian Shelf Model

Bundy.Scotian <-liste_mod[model.model_number == 40]

for(imod in 1:nrow(Bundy.Scotian)){
  mod.num <- as.numeric(as.character(Bundy.Scotian[imod, model.model_number]))
  h <- basicTextGatherer()
  curlPerform(url = paste0('http://sirs.agrocampus-ouest.fr/EcoBase/php/webser/soap-client_output.php?no_model=',
                           mod.num), writefunction = h$update, verbose = T)
  data <- xmlTreeParse(h$value(), useInternalNodes = T)
  ...
  if(length(model1 > 0)){
    ...
    ind.mod[, ModNum := mod.num]
    Bundy.Scotian <- ind.mod
  }
}

#####
Newfoundland.Heymans <-liste_mod[model.model_number == 108]

for(imod in 1:nrow(Newfoundland.Heymans)){
  mod.num <- as.numeric(as.character(Newfoundland.Heymans[imod, model.model_number]))
  h <- basicTextGatherer()
  curlPerform(url = paste0('http://sirs.agrocampus-ouest.fr/EcoBase/php/webser/soap-client_output.php?no_model=',
                           mod.num), writefunction = h$update, verbose = T)
  data <- xmlTreeParse(h$value(), useInternalNodes = T)
