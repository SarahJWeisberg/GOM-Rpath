...
#RPATH GOM groups
survey.groups <- as.data.table(unique(swept$RPATH, na.rm=true))
colnames(survey.groups)<-"RPATH"

#Remove multispecies groups
survey.groups<-survey.groups[RPATH %like% 'Other', RPATH := 'Other']
survey.groups<-subset(survey.groups[RPATH %notin% c('Other','SmPelagics','SouthernDemersals','SmFlatfish','SmFlatfishes','LargePelagics',),])
...
#OtherCeph.ba <-lm(Biomass~YEAR, data = OtherCeph.Biomass)
#spF <- as.numeric(summary(OtherCeph.ba)$fstatistic)
#p <- pf(spF[1], spF[2], spF[3], lower = F)
#ba <-as.numeric(OtherCeph.ba[[1]][2])
#OtherCeph.ba<-cbind('OtherCeph',ba,p)

#OtherSkates.ba <-lm(Biomass~YEAR, data = OtherSkates.Biomass)
#spF <- as.numeric(summary(OtherSkates.ba)$fstatistic)
#p <- pf(spF[1], spF[2], spF[3], lower = F)
#ba <-as.numeric(OtherSkates.ba[[1]][2])
#OtherSkates.ba<-cbind('OtherSkates',ba,p)
